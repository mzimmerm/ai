...
venv is Python build-in module; pyvenv is Python script on top of venv. pyenv
is a OS-level thing. All of them allow to create virtual environments
See
https://betterstack.com/community/questions/what-are-differences-between-python-virtual-environments/

Notes about pyenv:
- It allows to install multiple Python versions from source
- The multiple versions are installed in userspace (user home directory)
- User can switch between Python versions (todo-how)
- The restriction is that any Python version we try to install must compile on your system
- 

Status of Python on my system before installation:
...
There is only python3, python3.6, python3.6m, all linked to python2.6
...
#+BEGIN_SRC bash :results raw
mkdir --parent ~/software/python/pyenv3.10-for-rocm-pytorch
cd pyenv3.10-for-rocm-pytorch
sudo zypper install pyenv
#+END_SRC

List the versions pyenv can install for us.
...
Now we need to select the version we want to use for Pytorch. In that, we are limited to the version(s) for which AMD builds and tests their Pytorch-on-ROCm. These versions are available in todo
...
In our use of a ML project that uses PyTorch on ROCm, we will want to create a directory, say ~$ML_HOME~ in which our code will run using Python3.10.7. The following is a one-time command we need to run for any commands from ~$ML_HOME~ use Python3.10.7 in the future:

#+BEGIN_SRC sh :results raw
  ML_HOME=$HOME/dev/my-projects-source/public-on-github/ai/transformers/llm/train-bert-on-yelp
  cd $ML_HOME
  pyenv local 3.10.7
#+END_SRC
...
#+BEGIN_SRC sh :results raw
  ML_HOME=$HOME/dev/my-projects-source/public-on-github/ai/transformers/llm/train-bert-on-yelp
  # When in the directory for which we ran 'pyenv local 3.10.7', 3.10.7 is used
  cd $ML_HOME
  which python3
  python3 --version # 3.10.7
  python --version  # 3.10.7
  # When in the directory above, the system version is used
  cd ..
  python3 --version # 3.6.15
...
The functionality is achieved by the presence of the file ~.python-version~ in the directory ~$ML_HOME~, so do not delete it.

This corresponds to   Also, any Python imports run from there
