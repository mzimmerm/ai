...
# See prerequisites. Adding current user to Video and Render groups
sudo usermod -a -G render,video $LOGNAME
wget https://repo.radeon.com/amdgpu-install/6.0.2/ubuntu/${UBUNTU_RELEASE}/amdgpu-install_6.0.60002-1_all.deb
...
UBUNTU_RELEASE=focal # jammy is 22.04 focal is 20.04
		
#ROCM_VERSION_6060002=5.7.50703-1
#ROCM_VERSION_602=5.7.3
#AMDGPU_VERSION_602=$ROCM_VERSION_602
#ROCM_VERSION_60=5.7
...
# v4 have different naming conventions.
# Need to install Ubuntu 20.04.6 LTS Focal Fossa Changes March 23, 2023
# amdgpu-install_21.40.2.40502-1_all.deb
ROCM_VERSION_6060002=21.40.2.40502-1 #  amdgpu-install_21.40.2.40502-1_all.deb  
ROCM_VERSION_602=4.5.2
AMDGPU_VERSION_602=21.40.2
ROCM_VERSION_60=4.5 # this is torch, does not exist, will do manual
# 4. Add repositories

echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/rocm.gpg] https://repo.radeon.com/amdgpu/${AMDGPU_VERSION_602}/ubuntu ${UBUNTU_RELEASE} main" \
    | sudo tee /etc/apt/sources.list.d/amdgpu.list
sudo apt update --allow-insecure-repositories -y 

# ROCm repository
echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/rocm.gpg] https://repo.radeon.com/rocm/apt/${ROCM_VERSION_602} ${UBUNTU_RELEASE} main" \
...
AMDGPU_ROCM_PACKAGE_URL=https://repo.radeon.com/amdgpu-install/${AMDGPU_VERSION_602}/ubuntu/${UBUNTU_RELEASE}/${AMDGPU_ROCM_PACKAGE_FILE}
wget ${AMDGPU_ROCM_PACKAGE_URL}

# Ex: wget https://repo.radeon.com/amdgpu-install/6.0.2/ubuntu/${UBUNTU_RELEASE}/amdgpu-install_6.0.60002-1_all.deb
# Ex: wget https://repo.radeon.com/amdgpu-install/21.40.2/ubuntu/${UBUNTU_RELEASE}/amdgpu-install_21.40.2.40502-1_all.deb


# Install ROCm using amd install.
sudo apt update --allow-insecure-repositories
sudo apt install --yes ./$AMDGPU_ROCM_PACKAGE_FILE
# Ex: sudo apt install --yes ./amdgpu-install_5.7.50703-1_all.deb
sudo apt update --allow-insecure-repositories
...
* Note: Dependencies of torch, torchvision, torchaudio
...
* Using Anaconda instead of Pyenv to install and build Pytorch

** Install Anaconda in userspace; create and activate conda environment

Following steps in https://docs.anaconda.com/free/anaconda/install/linux/

# Install prerequisits Ubunt
sudo apt-get install libgl1-mesa-glx libegl1-mesa libxrandr2 libxrandr2 libxss1 libxcursor1 libxcomposite1 libasound2 libxi6 libxtst6
# Install prerequisites Tumbleweed - all already installed
...
# Install anaconda - software which installs Python and R.
# Apart from Python and R, it also provides other executables,
#   most importantly 'conda'. 
#   'conda' is a package manager (like pip), AND environment manager (line pyenv)
#
cd ~/tmp
PREFIX=~/software/anaconda
mkdir --parent $PREFIX
wget https://repo.anaconda.com/archive/Anaconda3-2024.02-1-Linux-x86_64.sh
bash ./Anaconda3-2024.02-1-Linux-x86_64.sh
#  - Running the above starts a command-line installer,
#    asking questions. I did the following:
#    1. Installed to ~/software/anaconda
#    2. Do you wish to update shell profile to automatically initialize conda? yes
#

#  - Start a new shell (to pick up changed path)!
#  - Perform checks below - all in the new shell.


# Check what Python is available in anaconda.
# (base) mzimmermann@acer-ryzen-laptop-wifi:~/tmp$ which conda
# /home/mzimmermann/software/anaconda/bin/conda
# (base) mzimmermann@acer-ryzen-laptop-wifi:~/tmp$ which python
# /home/mzimmermann/software/anaconda/bin/python
# (base) mzimmermann@acer-ryzen-laptop-wifi:~/tmp$ which python3
# /home/mzimmermann/software/anaconda/bin/python3
# (base) mzimmermann@acer-ryzen-laptop-wifi:~/tmp$ python --version
# Python 3.11.7
# (base) mzimmermann@acer-ryzen-laptop-wifi:~/tmp$ python3 --version
# Python 3.11.7


# Examples of using 'conda' as 'package manager'
#
#   - conda install scikit-learn-intelex

# Use 'conda' as 'environment manager' to create environment 
# named 'conda-env-torch-build-py310'. This environment contains fully 
# a separate installation of Python3.10, used when the 
# environment is 'activated'. See 'activation' below
# Consequences:
#   - Python3.10 is installed in:
#       ~/software/anaconda/envs/conda-env-torch-build-py310/lib/python3.10
#   - Python3.10 is on the path in:
#       ~/software/anaconda/envs/conda-env-torch-build-py310/bin
#   - Python3.10 packages are in:
#       ~/software/anaconda/envs/conda-env-torch-build-py310/lib/python3.10/site-packages
#   - After used activates conda-env-torch-build-py310,
#     this Python3.10 will be used. See next step re: activation
#

conda create --name conda-env-torch-build-py310 python=3.10

# Activate conda-env-torch-build-py310, which will have the following consequences while the enviroment is active:
#   - When typing 'python' or 'python3', Python3.10 in 
#       ~/software/anaconda/envs/conda-env-torch-build-py310/bin
#     is found and executed
#   - Packages installed (using conda or pip) will be copied to 
#        ~/software/anaconda/envs/conda-env-torch-build-py310/lib/python3.10/site-packages
#     The packages will be also found there 
#     when a Python program imports them in code such as 'import somepackage'.
#   - 
conda activate conda-env-torch-build-py310
...
g++ --version
# for some reason on Focal, we need this for setup.py to work:
sudo apt install g++ # Installs 9.4.0

# Activate the conda environment we prepared for torch building
conda activate conda-env-torch-build-py310

# Change to directory where we checkout and build torch
cd /home/mzimmermann/software/
...
git clone --recursive git@github.com:ROCm/pytorch # https://github.com/ROCm/pytorch
cd pytorch
git fetch --all --tags
git checkout origin/release/2.1_with_triton_fixes -b amd-release-2.1_with_triton_fixes
# the checkout has third_part now out of date - modified, so remove and re-checkout
# if you are updating an existing checkout - I think this should always run!!
git submodule sync                       # still clean after this command
git submodule update --init --recursive  # still clean after this command


# To get back to original state: 
# rm -r third_party torch aten c10 caffe2 binaries modules
# git checkout -- third_party torch aten c10 caffe2 binaries modules
# Also consider: git clean -fd .; git restore .; git submodule foreach --recursive git reset --hard; git checkout -- third_party


# Install python packages after clone and pull
conda install cmake ninja
conda install intel::mkl-static intel::mkl-include
pip install -r requirements.txt # still clean

# described as optional, but I did it. 
# Installs pytorch-triton in conda-env-torch-build-py310 site-packages
sudo apt install make
make triton

#Looking in indexes: https://download.pytorch.org/whl/nightly/
#Collecting pytorch-triton==2.1.0+e6216047b8
...
#Requirement already satisfied: filelock in /home/mzimmermann/software/anaconda/envs/conda-env-torch-build-py310/lib/python3.10/site-packages #(from pytorch-triton==2.1.0+e6216047b8) (3.13.1)
...
#Successfully installed pytorch-triton-2.1.0+e6216047b8


echo "Pull done. Next: build_amd.py and build install. Any key to continue: "
read

# Run AMD build first. changes stuff in caffe2, third_party etc.
# Changes at least: In Cmake*  __HIP_PLATFORM_HCC__ to  __HIP_PLATFORM_AMD__
...
# RUN SETUP: run 'setup.py develop' for quick turnaround
#                'setup.py build install' for one time

#NO export CFLAGS="$CFLAGS -D__HIP_PLATFORM_AMD__"
#NO export CXXFLAGS="$CXXFLAGS -D__HIP_PLATFORM_AMD__"

# setup options 
#   removed: USE_ROCM_KERNEL_ASSERT=1 \
#   NO NOT IN SETUP added:   TORCH_USE_HIP_DSA=1

# ONE MORE TRY: This article https://medium.com/@rafaelmanzanom/ditching-cuda-for-amd-rocm-for-more-accessible-llm-inference-ryzen-apus-edition-92c3649f8f7d tells us to use PYTORCH_ROCM_ARCH=gfx900 NOT gfx902 when building PYTORCH. ALSO they tell us to build magma. We do magma later. 
...
What is the difference between AMD architectures named gfx906 and gfx902?
Copilot
The gfx906 and gfx902 are identifiers for different GPU architectures from AMD.

The gfx902 architecture is associated with the Radeon Vega 8 GPU found in the AMD Ryzen 5 2500U1.

The gfx906 architecture, on the other hand, is associated with the AMD Instinct MI50 GPU2. This GPU provides 13.3 TFLOPs in FP32 operations2.

One of the key differences between these architectures is their support for the ROCm platform. According to AMD, the minimum requirement for ROCm is the gfx906 platform2, implying that gfx902 may not be officially supported.


** Build result: SUCCESS 2024-03-21. WARNINGS FROM LOG
 ...
 *Unfortunately, while build succeeded, the error is the same SO IT IS LIKELY NOT POSSIBLE TO CREATE A WORKING PYTORCH BUILD FOR gfx902*

 (conda-env-torch-build-py310) mzimmermann@acer-ryzen-laptop-wifi:~/software/pytorch$ HSA_OVERRIDE_GFX_VERSION=9.0.0 python -c "import torch; cuda0 = torch.device('cuda:0');print(torch.ones([2, 4], dtype=torch.float64, device=cuda0)); print('done')"
Traceback (most recent call last):
  File "<string>", line 1, in <module>
RuntimeError: HIP error: shared object initialization failed
HIP kernel errors might be asynchronously reported at some other API call, so the stacktrace below might be incorrect.
...
** Interesting from searching code

./.ci/pytorch/build.sh:  if [[ -n "$CI" && -z "$PYTORCH_ROCM_ARCH" ]]; then
./.ci/pytorch/build.sh:      echo "Limiting PYTORCH_ROCM_ARCH to gfx906 for CI builds"
./.ci/pytorch/build.sh:      export PYTORCH_ROCM_ARCH="gfx906"


* LATEST: INSTALL ROCM-DKMS and magma


sudo apt install libstdc++-13-dev rock-dkms rocm-dev rocm-libs miopen-hip rccl rocthrust hipcub roctracer-dev # cmake

THIS MAGMA BUILD NEVER WORKED
...
LANG=C.UTF-8 make lib/libmagma.so -j $(nproc) MKLROOT=/home/mzimmermann/software/anaconda
make testing/testing_dgemm -j $(nproc) MKLROOT=/home/mzimmermann/software/anaconda
#popd
#sudo mv magma /opt/rocm


Also checking device:

sudo find /sys/module/amdgpu/parameters/ -type f -name '*' -exec sh -c 'filename=${1%.*}; echo "File: ${filename##*/}"; cat "$1"' sh {} \;


ALSO DID: 
sudo modprobe amdgpu vramlimit=0 # REBOOT
# later: sudo modprobe amdgpu ppfeaturemask=0xfff73fff

* Downgrade jammy to focal

follow https://askubuntu.com/questions/49869/how-to-roll-back-ubuntu-to-a-previous-version

- edit sudo vi /etc/apt/sources.list, substitute jammy to focal
- sudo vi preferences.d/ubuntu-pro-esm-* change a=focal, priority 1001
- apt-get -fyu upgrade --allow-downgrades
- sudo apt autoremove
  
* Downgrade Ubuntu focal kernel to 5.6. Needed to install amdgpu 21.40.2. DO NOT DO THIS IF YOU CANNOT AFFORD TO BREAK YOUR SYSTEM BEYOND REPAIR.

# Using this link: https://askubuntu.com/questions/1404722/downgrade-kernel-for-ubuntu-22-04-lts

wget https://raw.githubusercontent.com/pimlie/ubuntu-mainline-kernel.sh/master/ubuntu-mainline-kernel.sh
chmod +x ubuntu-mainline-kernel.sh

# search and find your wanted version
ubuntu-mainline-kernel.sh -r | grep 5.6

# install that version kernel selected from the above list
sudo ./ubuntu-mainline-kernel.sh -i v5.6.19
...
# select the latest oem kernel, 5.6.0-1056-oem in my case
# change the grub configuration
sudo vi /etc/default/grub
from: GRUB_DEFAULT=0
to: GRUB_DEFAULT="Advanced options for Ubuntu>Ubuntu, with Linux 5.6.0-1056-oem"
