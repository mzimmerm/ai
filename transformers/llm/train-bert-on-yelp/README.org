...
amdgpu-install_5.3.50302-1_all.deb    
ROCM_VERSION_6060002=5.3.50302-1
ROCM_VERSION_602=5.3.2
AMDGPU_VERSION_602=$ROCM_VERSION_602
ROCM_VERSION_60=5.3
...
# 5. Download and install AMDGPU packages. DID NOT FIND LINUX-MODULES-EXTRA FOR MY LINUX VERSION
sudo apt install "linux-headers-$(uname -r)" "linux-modules-extra-$(uname -r)"
# See prerequisites. Adding current user to Video and Render groups
sudo usermod -a -G render,video $LOGNAME
...
sudo apt update --allow-insecure-repositories
#######
sudo apt-get --allow-unauthenticated  --allow-downgrades install --yes ./$AMDGPU_ROCM_PACKAGE_FILE
#######
...
pip uninstall torch torchvision torchaudio
# show what torch, torchvision, torchaudio depends on and consider uninstalling
...
# Install Anaconda prerequisits Ubuntu
sudo apt-get install libgl1-mesa-glx libegl1-mesa libxrandr2 libxrandr2 libxss1 libxcursor1 libxcomposite1 libasound2 libxi6 libxtst6
# Install Anaconda prerequisites Tumbleweed - all already installed
...
** Build result: SUCCESS 2024-03-21. BUT PYTORCH RUN FAILED: "HIP error: shared object initialization failed"

SO, *Unfortunately, while build succeeded, the error is the same SO IT IS LIKELY NOT POSSIBLE TO CREATE A WORKING PYTORCH BUILD FOR gfx902*
...
* Downgraded jammy to focal, to try AMDGPU 4.* version to see if that allows gfx900
...
sudo ./ubuntu-mainline-kernel.sh -i v5.4.272
...
# select the latest oem kernel, 5.4.272-0504272-generic in my case
# change the grub configuration
sudo vi /etc/default/grub
from: GRUB_DEFAULT=0
to: GRUB_DEFAULT="Advanced options for Ubuntu>Ubuntu, with Linux 5.4.272-0504272-generic"
