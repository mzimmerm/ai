...
** Installing PyTorch using the wheel whl files from AMD for ROCm 6.0.2. - FAILED with HIP error: shared object initialization failed
...
** Installing PyTorch using NIGHTLY for ROCm 6.0.2 - Fails ERROR: HSA_STATUS_ERROR_MEMORY_APERTURE_VIOLATION: The agent attempted to access memory beyond the largest legal address.


ROCM_TORCH_WHL="--index-url https://download.pytorch.org/whl/nightly/rocm6.0"
cd HUGE-NO-BACKUP
pip3 install --force-reinstall --pre torch torchvision  $ROCM_TORCH_WHL
