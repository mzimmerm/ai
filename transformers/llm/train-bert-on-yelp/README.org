...
# The amdgpu-install version AMDGPU_ROCM_PACKAGE amdgpu-install-6.0.60002 is version of ROCm
# The ROCm version must match the version in TORCH_ROCM_WHL - rocm6.0
# The "cp310" in TORCH_ROCM_WHL describes Python 3.10 
AMDGPU_ROCM_PACKAGE=https://repo.radeon.com/amdgpu-install/6.0.2/sle/15.5/amdgpu-install-6.0.60002-1.noarch.rpm

# Install ROCm using amd install.
# Should work now when packages added.
# sudo zypper --no-gpg-checks install $AMDGPU_ROCM_PACKAGE
...
# TORCH_ROCM_WHL=https://repo.radeon.com/rocm/manylinux/rocm-rel-6.0.2/torch-2.1.2+rocm6.0-cp310-cp310-linux_x86_64.whl

# Install Torch from nightly wheel files.
# Alternative for TORCH_ROCM_WHL is to specify nithgtly torch builds; they have WHL files for rocm6.0-cp310. Those seem to be automatically selected when we specify simply:
TORCH_ROCM_WHL="--index-url https://download.pytorch.org/whl/nightly/rocm6.0"
cd HUGE-NO-BACKUP
pip3 install --force-reinstall --pre torch torchvision torchaudio $TORCH_ROCM_WHL
...
TORCH_ROCM_WHL="--index-url https://download.pytorch.org/whl/nightly/rocm6.0"
cd HUGE-NO-BACKUP
pip3 install --force-reinstall --pre torch torchvision  $TORCH_ROCM_WHL
...
AMDGPU_ROCM_PACKAGE=https://repo.radeon.com/amdgpu-install/${ROCM_VERSION_602}/ubuntu/jammy/${AMDGPU_ROCM_PACKAGE_FILE}
...
#TORCH_ROCM_WHL=https://repo.radeon.com/rocm/manylinux/rocm-rel-6.0.2/torch-2.1.2+rocm6.0-cp310-cp310-linux_x86_64.whl
 TORCH_ROCM_WHL_FILE=torch-1.13.1+rocm5.7-cp39-cp39-linux_x86_64.whl
 TORCH_ROCM_WHL=https://repo.radeon.com/rocm/manylinux/rocm-rel-5.7/$TORCH_ROCM_WHL_FILE
 TORCHVISION_ROCM_WHL_FILE=torchvision-0.14.1+rocm5.7-cp39-cp39-linux_x86_64.whl
 TORCHVISION_ROCM_WHL=https://repo.radeon.com/rocm/manylinux/rocm-rel-5.7/$TORCHVISION_ROCM_WHL_FILE

wget $TORCH_ROCM_WHL
wget $TORCHVISION_ROCM_WHL
